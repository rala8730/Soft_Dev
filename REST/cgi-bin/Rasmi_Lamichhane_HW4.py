...
#3 things to study 
#html forms
#http get and post methodes
#url="http://api.openweathermap.org/data/2.5/weather?q=boulder&APPID=b8d83bea990db8c592280cfc402fe4c5"
# for loop for the url, data,response

#data = urllib.urlopen(url).read()
#response = json_loads_byteified(data)
...
'''
################11) Dictionary of the state_cap #################################
states_caps = {
...
################9)adding the color for boulder co #######################

#print "$('#CO').css('fill', 'red')"

##############num 12 for loop #########
for statename in states_caps.keys():
	#print "$('#" + statename + "').css('fill','red')"
#dividing the string into 3 parts ==> "" before $ && statename && next ""
###########addition in for loop fot every other state########3333
	#print "$('#" + statename + "').css('fill','color')"
	city= states_caps[statename]['capital']
        city=city.replace(' ','+')
	url="http://api.openweathermap.org/data/2.5/weather?q="+city+"&APPID=b8d83bea990db8c592280cfc402fe4c5"
	data = urllib.urlopen(url).read()
	response = json_loads_byteified(data)
